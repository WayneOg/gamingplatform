...
export const TournamentCard = ({ tournament, onRemove }) => {
  const timeRemaining = tournament.startTime - Date.now();
  const days = Math.floor(timeRemaining / (1000 * 60 * 60 * 24));
  
  const handleRemove = () => {
      if (onRemove) {
          onRemove(tournament.id); // Assuming each tournament has a unique id
      }
  };
    ...
    <div className="bg-white rounded-xl shadow-lg overflow-hidden">
      ...
      <div className="p-4">
        ...
        <button 
          ...
          className="mt-2 w-full bg-red-500 hover:bg-red-600 text-white py-2 rounded-lg transition-all duration-300 transform hover:scale-105"
        >
          Remove Tournament
        </button>
      </div>
